            ...
            if new_suppression_count == 0:
                ...
                if old_suppression_count == new_suppression_count: 
                    ...
                    for old, new in zip(old_type_line_set, new_type_line_set):
                        old_warning_types, new_warning_types = separate_multiple_warning_types(old, new)
                        ...
                        type_line_set, operation_set = identify_inline_change_operation(old_multi_num, new_multi_num, \
                                old, new, old_warning_types, new_warning_types)
                else: # Include tricky cases, and some normal delete and add cases.
                    type_line_set, operation_set, tricky_mark = identify_complex_change_operations(old_type_line_set, new_type_line_set)
        return type_line_set, operation_set, tricky_mark   


def separate_multiple_warning_types(old, new):
...
def identify_inline_change_operation(old_multi_num, new_multi_num, old, new, old_warning_types, new_warning_types):
    ...
    if old_multi_num == new_multi_num:
        ...
        for old_type in old_warning_types:
            ...
            if old_type not in str(new_warning_types):
                ...
                try:
                    new_warning_types.remove(old_type) # No change
...
def identify_complex_change_operations(old_type_line_set, new_type_line_set):
    # This function is called under condition - old_suppression_count != new_suppression_count
    type_line_set = []
    operation_set = []
    tricky_mark = False
