...
parser = argparse.ArgumentParser(description="Compute mapping between warnings and suppressions")
parser.add_argument("--repo_dir", help="Directory with the repository to analyze", required=True)
parser.add_argument("--commit_id", help="Commit hash to analyze", required=True)
parser.add_argument("--lang", help="The programming language of the code to analyze", required=True, choices=["python"])
parser.add_argument("--checker", help="The checker to run", required=True, choices=["mypy", "pylint"])
...
def get_all_warnings(repo_dir, commit_id, checker, results_dir):
    ...
    # read them into a list
    warning_file = join(results_dir, "checker_results", commit_id, f"{commit_id}_warnings.csv")
    warnings = read_warning_from_file(warning_file)
    return warnings

def write_results_to_csv(suppression_warning_pairs, results_dir, commit_id):
    with open(join(results_dir, f"{commit_id}_mapping.csv"), "w") as f:
        writer = csv.writer(f)
        for suppression, warning in suppression_warning_pairs:
            if warning is None:
                writer.writerow([suppression.path, suppression.text, suppression.line, "", "", ""])
            else:
                writer.writerow([suppression.path, suppression.text, suppression.line, warning.path, warning.kind, warning.line])
...
def main(repo_dir, commit_id, lang, checker, results_dir):
    ...
    original_warnings = get_all_warnings(repo_dir, commit_id, checker, results_dir)
    ...
    for suppression in suppressions:
        ...
        remover.restore()
    write_results_to_csv(suppression_warning_pairs, results_dir, commit_id)


if __name__=="__main__":
    args = parser.parse_args()
    main(args.repo_dir, args.commit_id, args.lang, args.checker, args.results_dir)
